...
async function fetchYears() {
  try {
    ...
    const response = await fetch(
      "http://localhost/voting_system/server/controller/results/get_examyear.php?action=read"
    );
...
function selectYear(year) {
  ...
  document.querySelectorAll(".year-btn").forEach((btn) => {
    btn.style.backgroundColor =
      btn.textContent === year ? "#0056b3" : "#007bff";
  });
      ...
      filteredResults.forEach((result) => {
        html += `
                    <tr>
                        ...
                        <td>${result.nic}</td>
                        <td>${result.results
                          .map((r) => `Subject ${r.subject_id}: ${r.result}`)
                          .join("<br>")}</td>
                    </tr>
                `;
      });
