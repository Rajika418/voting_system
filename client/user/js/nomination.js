document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('http://localhost/voting_system/server/controller/election/election_get.php')
        ...
        .then(data => {
            if (data.status === 'success' && data.data.length > 0) {
                ...
                document.getElementById('election-name').textContent = election.election_name;
                document.getElementById('election-date').textContent = `Election Date: ${election.ele_start_date} - ${election.ele_end_date}`;
                document.getElementById('nomination-period').textContent = `Nomination Period: ${election.nom_start_date} - ${election.nom_end_date}`;
    ...
    document.getElementById('student-id').addEventListener('input', function() {
        ...
        if (regNo) {
            fetch(`http://localhost/voting_system/server/controller/election/nomination/reg_no_get.php?registration_number=${regNo}`)
                .then(response => response.json())
                .then(data => {
                    if (data.status === 'success') {
                        // Display student info on the form
                        document.getElementById('student-info').textContent = `Student: ${data.student_name}, Grade: ${data.grade_name}`;
                        document.getElementById('hidden-student-id').value = data.student_id;
                    } else {
                        document.getElementById('student-info').textContent = "Student not found.";
                    }
                })
