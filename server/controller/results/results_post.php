...
try {
    ...
    if (!empty($data)) {
        ...
        // Loop through the data and insert each result
        ...
        if ($successful_inserts == count($data)) {
            ...
            echo json_encode(["message" => "All results were submitted successfully."]);
        } else {
            http_response_code(503);
            echo json_encode(["message" => "Unable to submit all results. Some index_no values may not exist in the student_exam table."]);
        }
    } else {
        http_response_code(400);
        echo json_encode(["message" => "Unable to submit results. Data is incomplete."]);
    }
} catch (PDOException $e) {
    http_response_code(503);
    echo json_encode(["message" => "Unable to submit results: " . $e->getMessage()]);
}
